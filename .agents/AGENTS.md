...
sirket-veritabani/
├── scrapers/
│   ├── utikad_scraper.py          # UTİKAD üye listesi → sirketler.csv
│   ├── und_scraper.py             # UND üye listesi → und_sirketler.csv
│   ├── merge_and_dedup.py         # Kaynak CSV'leri birleştirir → master_sirketler.csv
│   ├── final_serpapi_scraper.py   # LinkedIn yönetici araması (SerpAPI) → yoneticiler_final.csv
│   └── website_scraper_legacy/    # Şirket sitelerinden yönetici kazıma (legacy)
├── database/
│   ├── models.py                  # SQLAlchemy modelleri (Sirket, Kisi)
│   └── connection.py              # PostgreSQL / SQLite bağlantısı
├── master_sirketler.csv           # Birleştirilmiş ana şirket listesi (git'te izlenir)
├── und_sirketler.csv              # UND kaynağı (git'te izlenir)
├── requirements.txt
└── .env.example
```
...
```bash
python scrapers/utikad_scraper.py
python scrapers/und_scraper.py
python scrapers/merge_and_dedup.py
python scrapers/final_serpapi_scraper.py
```
